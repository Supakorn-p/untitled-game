...
flowchart TD
    subgraph SOURCES["แหล่ง Asset ฟรี"]
        K1[Kenney.nl / OpenGameArt / itch.io] --> C1[docs/assets/_candidates/]
        K2[Freesound.org] --> C1
        K3[Google Fonts / Fontesk] --> C1
    end

    subgraph REPO["Git Repository (Staging)"]
        C1 --> R1[docs/assets/_candidates/sprites]
        C1 --> R2[docs/assets/_candidates/sfx]
        C1 --> R3[docs/assets/_candidates/music]
        C1 --> R4[docs/assets/_candidates/fonts]
    end
